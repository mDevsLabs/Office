...
/**
 * mAI Office unified shell: ONE Electron app, ONE BrowserWindow, hosting the
...
// run silently quits and forwards its argv to the running installed mAI Office.
...
if (!app.isPackaged)
  app.setPath(
    'userData',
    process.env.GENOFFICE_USER_DATA ?? join(app.getPath('appData'), 'mAI Office Dev'),
  )

// The product rename from GenOffice to mAI Office changed the userData path; migrate old user data once
if (app.isPackaged) {
  const oldDir = join(app.getPath('appData'), 'GenOffice')
...
const tMain = createI18n({
  zh: {
    ...
    menuSectionNew: '新建',
    menuNewDoc: 'AI Docs',
    menuNewSheet: 'mAI Office Sheets',
    untitledSheet: '未命名表格',
    untitledDoc: '未命名文档',
    untitledDeck: '未命名演示文稿',
    menuNewSlide: 'mAI Office Slides',
    menuOpen: '打开…',
    menuSave: '保存',
    menuSaveAs: '另存为…',
    menuClose: '关闭',
...
function registerHomeIpc(): void {
  // mAI Office account (gsk login state; to be upgraded to a signup/account system later)
