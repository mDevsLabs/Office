...
export interface SlidesApi {
  ...
  /** mAI Office account status (gsk login state); with withEmail also fetches the email (needs a network request, slower) */
  aiGskStatus: (withEmail?: boolean) => Promise<GenSparkAccountStatus>
  /** Open the browser to log into mAI Office (fire-and-forget; aiGskStatus turns logged-in once done) */
  ...
  /** gsk (mAI Office) AI image generation/editing, returns the image URL (error prompts login when logged out) */
  generateImage: (op: {
    prompt: string
    model?: string
  ...
  /** gsk (mAI Office) media analysis: image/audio/video content understanding, returns analysis text */
  analyzeMedia: (op: {
    mediaUrls: string[]
    requirements: string
